...
String WebSocketServer::handleStream() {
    ...
    if (socket_client->connected() && socket_client->available()) {
        ...
        dataSize = length;
        for (i=0; i<length; ++i) {
            _val = (timedRead() ^ mask[i % 4]);
            socketString += (char) _val;
            buffer[i] = _val;
            if (!socket_client->connected()) {
                return socketString;
            }
...
// added to read raw bytes

int WebSocketServer::parsePacket(){
    // clear the datasize and buffer
    dataSize = 0;
    memset(buffer, 0, sizeof(buffer));
    // handle the stream
    if (hixie76style) {
...
void WebSocketServer::readBytes(uint8_t * _array, size_t _len){
    memcpy(_array, buffer, _len);
}
